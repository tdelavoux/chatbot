    ...
    public function reciveDemande(Request $request){
        ...
        foreach(self::NICE_STUFF as $nice){
            if(str_contains($pattern , $nice)){
                $result = view('AppViews.partials.lovelyMessage')->render();
                break;
            }
        }
        ...
        foreach(self::BAD_STUFF as $bad){
            if(str_contains($pattern , $bad)){
                $result = view('AppViews.partials.angryMessage')->render();
            }
        }
        
        if(!$result){
            $appProxi = Proximite::searchProxi($pattern);
            $result = view('AppViews.partials.appResponse')->with('app', $appProxi)->render();
        }

        return response()->json([
            'userView' => view('AppViews.partials.userChat')->with('pattern', $request->pattern)->render(),
            'typing'   => view('AppViews.partials.typing')->with('demandId', $uniquid)->render(),
            'response' => $result,
            'demandId' => $uniquid,
        ]);
    }
